...
func main(){
	...
	Используя этот паттерн, мы позволяем писать 'новые' методы для структур, при этом не изменяя их. Изменять/создавать новые методы мы будем в реализации интерфейса Visitor.
	Мы так-же сможем изменять поведение метода у посетителя, не изменяя другие классы.
	Из мунусов могу подметить то, что повышается сложность программы, если применять это всегда и везде.
	*/
}
